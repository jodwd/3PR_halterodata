...
max_year = (datetime.today() + relativedelta(months=4)).year
selected_year = [min_year, max_year]

# Requête principale
# qry = """
#    SELECT *
#    FROM REPORT_ATHLETES
#    WHERE SaisonAnnee BETWEEN :min_year AND :max_year
#    """
...
df2['Série'] = pd.Categorical(df2['Série'],
                              ["N.C.", "DEB", "DPT", "REG", "IRG", "FED", "HON", "NAT", "INT B", "EUR", "INT A", "MON",
                               "OLY"], ordered=True)
df_temp = df[df['Nom'] == 'ZZZZZ']
dash.register_page(__name__, path='/', name='3PR - Athletes', title='3PR - Perfs Athlètes', image='/assets/3PR.png',
...
layout = html.Div([
    ...
    dbc.Row([
        html.P("MàJ : " + max_date, id="zone_news", className="news")
    ]),
    dbc.Row([  # Titre
        dbc.Col([
                            ...
                            dbc.Modal([
                                dbc.ModalHeader([
                                    ...
                                    html.Div([
                                        dbc.Row([
                                            dbc.Col([
                                                dbc.Button("❓", id="open-aide_achievements-athl", color="white",
                                                           className="mt-auto", size="sm"),
                                                dbc.Modal([
                                                    dbc.ModalHeader([
                                                        dbc.ModalTitle("Information",
                                                                       id="achievement_aide_header_athl"),
                                                    ]),
                                                    dbc.ModalBody([
                                                    ...
                                                    dbc.ModalFooter(
                                                        dbc.Button("Fermer", id="close-aide_achievements-athl",
                                                                   color="secondary", className="ml-auto")
                                                    ),
                                                ], id="aide_achievements_athl", size="md", centered=True,
                                                    is_open=False),
                                            ]),
                                        ], align="right"),
                                ...
                                dbc.ModalBody([
                                    dbc.Button("Voir les Athlètes Similaires", id="athl_sim", color="primary",
                                               className="mt-auto", size="sm"),
...

                {
                    ...
                    "children": [
                        ...
                        {"field": "IWF", "width": 80, "hide": False,
                         "valueFormatter": {"function": "params.value ? params.value.toFixed(3) : ''"}},
...
def update_table_athl4(txt_inserted, is_open_athl, is_open_athl_sim, open_clicks1, open_clicks2, open_clicks3, open_clicks4):
    ...
    df1 = df[
        (df['Nom'] == athl) & (df['SaisonAnnee'] >= min(selected_year)) & (df['SaisonAnnee'] <= max(selected_year))]
    df1 = df1.sort_values(by=['Date'], ascending=False)
    date_naiss = df1['Né le'].values[0]

    up_achievements = []
    if df1['MondeSEN'].values[0] > 0:
        up_achievements.append(str(df1['MondeSEN'].values[0]) + ' x 🌎 | ')
    if df1['MondeU20'].values[0] + df1['MondeU17'].values[0] > 0:
        up_achievements.append(str(df1['MondeU20'].values[0] + df1['MondeU17'].values[0]) + ' x 🌎🐥 | ')
    if df1['MondeMasters'].values[0] > 0:
        if df1['Sexe'].values[0] == 'F':
            up_achievements.append(str(df1['MondeMasters'].values[0]) + ' x 🌎👵 | ')
        else:
            up_achievements.append(str(df1['MondeMasters'].values[0]) + ' x 🌎👴 | ')
    if df1['EuropeSEN'].values[0] > 0:
        up_achievements.append(str(df1['EuropeSEN'].values[0]) + ' x 🇪🇺 | ')
    if df1['EuropeU23'].values[0] + df1['EuropeU20'].values[0] + df1['EuropeU17'].values[0] > 0:
        up_achievements.append(str(df1['EuropeU20'].values[0] + df1['EuropeU17'].values[0]) + ' x 🇪🇺🐥 | ')
    if df1['FranceElite'].values[0] > 0:
        up_achievements.append(str(df1['FranceElite'].values[0]) + ' x 🇫🇷 | ')
    if df1['GrandPrixFederal'].values[0] > 0:
        up_achievements.append(str(df1['GrandPrixFederal'].values[0]) + ' x 🐔 | ')
    if df1['TropheeNationalU13'].values[0] > 0:
        up_achievements.append(str(df1['TropheeNationalU13'].values[0]) + ' x 🇫🇷👶 | ')
    if df1['MondeMasters'].values[0] > 0:
        if df1['Sexe'].values[0] == 'F':
            up_achievements.append(str(df1['MondeMasters'].values[0]) + ' x 🌎👵 | ')
        else:
    ...
    if df1['EuropeMasters'].values[0] > 0:
        if df1['Sexe'].values[0] == 'F':
            up_achievements.append(str(df1['EuropeMasters'].values[0]) + ' x 🇪🇺👵 | ')
        else:
            up_achievements.append(str(df1['EuropeMasters'].values[0]) + ' x 🇪🇺👴 | ')
    if df1['NbCompet'].values[0] == 100:
        up_achievements.append(' 💯+ comps | ')
    if df1['NbCompet'].values[0] == 50:
        up_achievements.append(' 5️⃣0️⃣+ comps | ')
    if df1['Nb6sur6'].values[0] > 0:
        up_achievements.append(str(df1['Nb6sur6'].values[0]) + ' x 👌 | ')
    if df1['Nb2sur6DerniereBarre'].values[0] > 0:
        up_achievements.append(str(df1['Nb2sur6DerniereBarre'].values[0]) + ' x 🫣 | ')
    if df1['NbBulles'].values[0] > 0:
        up_achievements.append(str(df1['NbBulles'].values[0]) + ' x 🔴 | ')
    if df1['NbDoublesBulles'].values[0] > 0:
        up_achievements.append(str(df1['NbDoublesBulles'].values[0]) + ' x 🔴🔴 | ')
    if up_achievements:  # TODO
        ' - '.join(up_achievements)
    ...
    df2_athl['Série'] = pd.Categorical(df2_athl['Série'],
                                       ["N.C.", "DEB", "DPT", "REG", "IRG", "FED", "HON", "NAT", "INT B", "EUR",
                                        "INT A", "MON", "OLY"],
                                       ordered=True)
    ...
    fig_athl = px.histogram(df2_athl, x="Série", color="Catégorie",
                            color_discrete_sequence=["#DC4C64", "#3B71CA", "#E4A11B", "#14A44D", "#FBFBFB", "purple",
                                                     "#54B4D3", "#9FA6B2"],
                            category_orders={
                                "Série": ["N.C.", "DEB", "DPT", "REG", "IRG", "FED", "HON", "NAT", "INT B", "EUR",
                                          "INT A", "MON", "OLY"]})
...
def update_table_athl1(is_open_ach):
    ...
    if is_open_ach:
        return [
            dbc.Table.from_dataframe(df_ach, responsive=True, striped=True, bordered=True, hover=True)], ach_aide_txt
